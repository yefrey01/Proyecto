 ...
 *   https://datatables.net/download/#dt/jqc-1.11.3,dt-1.10.9,r-1.0.7
 *
 * Included libraries:
...
jQuery.fn.dataTable.ext.builder = "dt\/jqc-1.11.3,dt-1.10.9,r-1.0.7";
...
$(document).ready(function(){
	...
	// Inicio proceso de Validación formulario gestión procesos
	var conten_errores = $('.errores');

		$("#cerrarmodulacion").on("ifChecked",function(){
				$('#mtrtotalesmtr2').prop('required',true);
				$('#valortotalmtr2').prop('required',true);
		});		
	 ...
	 // Final Modulación formulario gestión de procesos
	 ...
	 $(".frmpass").validate({
	  ...
	  submitHandler: function(form) {
			var var_pass1=$('#txtcanterior').val();
			var var_pass2=$('#txt_cnueva1').val();
			var var_pass3=$('#txt_cnueva2').val();

			var toLoad= 'controlador.php?accion=1&pass1='+var_pass1+'&pass2='+var_pass2+'&pass3='+var_pass3;
			$.post(toLoad,function(responseText){
				$('.errores ul li').remove();
				if(responseText.trim()=='1111'){
					form.submit();				
				}else{
					if(responseText.trim()=='2222'){
						$('.errores,.revisar ul').css("display","block");					
						$('.revisar ul').append('<li>Las nuevas contraseñas no son iguales</li>');
					}else{
						if(responseText.trim()=='3333'){
							$('.errores,.revisar ul').css("display","block");
							$('.revisar ul').append('<li>La contraseña anterior con corresponde</li>');	
						}						
					}
				}
			});
	...
	function filterGlobal () {
	  $('#tabla_cotizacion').DataTable().search(
	   $('#global_filter').val()
	  ).draw();

	  $('#tabla_seguimiento').DataTable().search(
	   $('#global_filter').val()
	  ).draw();
	...
	var oTable = $("#tabla_cotizacion").dataTable();
	 oTable.on('click', '.editar',function(e){ 
	  e.preventDefault();
	  $(this).parent('.editarficha').submit();
	 });

	var oTable = $("#tabla_menu").dataTable();
	 oTable.on('click', '.editar',function(e){ 
	  e.preventDefault();
	  $(this).parent('.editarficha').submit();
	 });

	var oTable = $("#tabla_seguimiento").dataTable();
	 oTable.on('click', '.agregarmenu',function(e){ 
	  e.preventDefault();
	  $(this).parent('.agregarseg').submit();
	 });
	...
	//$( ".fecha" ).datepicker({ minDate: -1, maxDate: "+1M +10D" });
	$( ".fecha" ).datepicker({ minDate: 0, maxDate: "+24M" });
	$(".fecha").datepicker({ 
		dateFormat: 'yy-mm-dd'
	});
	...
	function grupo(){
		$(".grupo").each(function(){
			...
			$(this).find("div[class*='col-']").css("height", alto);
			//console.log(alto);
	...
	$(document).on('click', '.cerrar, .fancybox-overlay',function(){
		$.fancybox.close();
	});
	...
	$('.fancybox-ajax').fancybox({
		...
		beforeShow: function(){
				...
				$('.tabla--box-buscar tfoot th').each( function () {
					var title = $(this).text();
